...
function [mims] = multimg_matches(pmatches)
  ...
  nframes=length(pmatches)+1; % #frames  % get the number of the frames.

  % count the pairwise matches and find the matched feature ids in every frame
  npmatches=zeros(nframes, nframes);  % 记录图像帧对之间匹配的特征数量。
  feats=cell(nframes, 1);   % 用来记录每帧的所有匹配特征ID。
  for i = 1:nframes-1
    for j = i+1:nframes
      pmatchesij = pmatches{i}{j}; % Get the current matched feature ID.
      if isempty(pmatchesij), continue; end
      npmatches(i, j)  = size(pmatchesij, 2); % Get the matched feature number of current frame pair.
      feats{i} = union(feats{i}, pmatchesij(1,:));   % 累计不重复记录每帧内的匹配特征特征ID号。
      feats{j} = union(feats{j}, pmatchesij(2,:));
    end
  end

  nfeat = zeros(nframes, 1); % #features per frame
  for i = 1:nframes, nfeat(i) = length(feats{i}); end  % 记录每帧内的特征点数量。

  frstart = [0; cumsum(nfeat)]; % offset of each frame in ftinfr (see below); size nframes+1
  totfeat = frstart(nframes+1);  % 所有帧内的特征点数量。
  ...
  ftinfr = zeros(totfeat, 1);  % ftinfr 中记录了所有帧内匹配特征的编号。
  for k = 1:nframes
    for i = 1:nfeat(k)
...
tic;
  ...
  G=sparse([],[],[], totfeat, totfeat, totpmatches);  % 产生812*812的稀疏矩阵，共计686非零元素。
  for i=1:nframes-1
    ...
    for j=i+1:nframes
      ...
      for k=1:length(pmatchesij)
        m=pmatchesij(:, k);
        ii=binsearch(ftinfr, frstart(i), frstart(i+1), m(1));  % 查找元素对应的序号。
        jj=binsearch(ftinfr, frstart(j), frstart(j+1), m(2));  % 查找元素对应的序号。
...
tic;
  ...
  [rp, ci]=sparse_to_csr(G+G'); % adding G' makes the CRS matrix undirected! 将有向图变为无向图。
